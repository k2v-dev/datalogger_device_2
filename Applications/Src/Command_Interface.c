...
uint8_t length;
uint16_t checksum;
accel3x_values_t  accel3_values;
...
void calculate_timestamp(void)
{
	...
	data_array_output[k++]= (uint8_t)milisec;
...
void cmd_send_acc_3_axis(void)
{
	...
	calculate_timestamp();
	read_IMU3axis();
	ax=~accel3_values.x*780;
	ay=~accel3_values.y*780;
	az=~accel3_values.z*780;
	NRF_LOG_INFO("Sample # %d\r\nAX: %06d\r\nAY: %06d\r\nAZ: %06d ",
				 sample++,(ax) ,(ay), (az)); //accl values
	data_packets(ax);
	data_packets(ay);
	data_packets(az);
	calculate_checksum();
...
void cmd_send_all_SENSOR(void)
{
	int16_t ax,ay,az,gx,gy,gz,mx,my,mz,ax2,ay2,az2;
	...
	my=(~mag_values.my+1);
	mz=(~mag_values.mz+1);
	ax2=(~accel3_values.x)*780;
	ay2=(~accel3_values.y)*780;
	az2=(~accel3_values.z)*780;
	data_packets(ax);
	data_packets(ay);
	data_packets(az);
	data_packets(gx);
